...
Puoi fare il deploy del tuo sito Astro su [Deno Deploy](https://deno.com/deploy) utilizzando il server-side-rendering (SSR). Ricorda che Deno Deploy non supporta i siti statici.
...
1. (Optional) Per semplificare questo processo per eseguire il deploy, aggiungi uno script `deploy-deno` nel tuo `package.json`.
...
📚 Scopri di più sul [SSR in Astro](/it/guides/server-side-rendering/).
